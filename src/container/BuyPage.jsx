  ...
  useEffect(() => {
    const FetchData = async () => {
      try {
        ...
        const TeleFunInfo = await readContract({
          address: TeleFunAddress,
          abi: TeleFunAbi,
    ...
    try {
      if (TeleFunAddress) {
        await fetch(
          ...
          `https://api.routescan.io/v2/network/testnet/evm/${chainId}/address/${TeleFunAddress}/transactions`,
          {
            method: 'GET'
          }
        ).then(async res => {
                  ...
                  <div className="claim-card p-6">
                    ...
                    <section className="flex flex-col gap-6 mt-4">
                      ...
                      {address === undefined ? (<button onClick={onConnectWallet} className='text-[16px] focus:outline-none h-[48px] flex justify-center items-center select-none font-bold text-center w-full bg-[#2fcdf3] hover:opacity-90 disabled:bg-[#192d1c] disabled:text-[#bbb] rounded-[24px] text-[#222]'>
                        ...
                        :
                        <button onClick={() => switchNetwork?.(chainId)} className='text-[16px] focus:outline-none h-[48px] flex justify-center items-center select-none font-bold text-center w-full bg-[#2fcdf3] hover:opacity-90 disabled:bg-[#192d1c] disabled:text-[#bbb] rounded-[24px] text-[#222]'>
                          Switch Network
                        </button>}
                    </section>
                  </div>

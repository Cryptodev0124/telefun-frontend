...
import formatNumber, { imageUrl, apiUrl, supportedChainIds } from '../utils/constants.ts'
...
const TopBar = (animate) => {
  ...
  useEffect(() => {
    ...
    if (isConnected === true) {
      if (!supportedChainIds.includes(chain?.id)) switchChain()
    }
  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 w-full z-50 px-2">
        <div className={`my-4 max-w-7xl m-auto w-full bg-[#2fcdf3] rounded-[25px] lg:rounded-full lg:h-[70px] items-center ${isExpanded ? 'pb-2 px-2' : 'px-2'}`} style={{ boxShadow: "rgb(0, 0, 0) 2px 4px 0px 0px, rgb(255, 255, 255) 3px 4px 0px 0px inset" }}>
          <div className='flex justify-between'>
            <div className='flex flex-row items-center'>
              <TeleFunHeaderLink className="w-[56px] h-[56px]" />
              <div className='lg:flex hidden flex-row gap-4' style={{ fontFamily: "Dela Gothic One" }}>
                <Link to="/AllLaunches" className="left-bar-link">
                  <span
                    className={
                      currentPath === '/' || currentPath === '/AllLaunches'
                        ? 'text-[20px] uppercase text-[#1b371f]'
                        : 'text-[20px] uppercase text-[#222] hover:text-[#1b371f]'
                    }
                  >
                    Board
                  </span>
                  ...
                  <section>
                    <div className="ChainGroupButton">
                      {supportedChainIds.includes(chain?.id) ? (
                        ...
                        >
                          {'Wrong Network'}
                          {isLoading && pendingChainId === 11155111 && ''}
                        </button>
                      )}
                    </div>
                  </section>
